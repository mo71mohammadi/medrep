...
def get_company(company):
    companyDB.update_one({"_id": company['_id']}, {"$set": {"update": True}})
    sleep(CompanyList.index(company) / 10)
    print(CompanyList.index(company))
    item_request_body = {
    ...
    #     if company['start'] != -1 & list_0.index(item) + 1 >= int(company['start']):
    ...
    #             "UMDNS": fields[4], "manuName": fields[5],
    #             "countryLegal": fields[6], "eqType": fields[7],
    #         }
    #
    #         # ..................... START LEVEL ONE ........................... #
    ...
    #         except:
    #             companyDB.update_one({"_id": company['_id']}, {"$set": {"update": False}})
    #             return repeat()
    ...
    #                     FANI[obj[0]] = object
    #             IRC_GTIN = re.findall(
    #                 r'''</td><td>(.*)</td><td><a title="(.*)">(.*)</a></td><td>(.*)</td><td>(.*)</td>''',
    #                 level_3_irc.text)
    #             temp = re.findall(
...
def repeat():
    select = Company[0]
    Company.remove(select)
    get_company(select)


with ThreadPoolExecutor(max_workers=500) as pool:
    pool.map(get_company, select_0)
